...
Computational resource settings selected in the configurator are written into each sample's `nextflow.config.backup` before export. When a sample pipeline directory is first created, the runner copies that backup into `CRISPR_Pipeline/nextflow.config`. To change CPUs, memory, retries, Singularity timeout, or GPU queues before the pipeline directory exists, edit the relevant sample `nextflow.config.backup`. After `CRISPR_Pipeline` exists, edit `CRISPR_Pipeline/nextflow.config` or regenerate the ZIP from the configurator.
