...
Computational resource settings selected in the configurator are written into each sample's `nextflow.config` before export. When a sample pipeline directory is first created, the runner copies that config into `CRISPR_Pipeline/nextflow.config`, then renames the sample-level file to `nextflow.config.backup`. To change CPUs, memory, retries, Singularity timeout, or GPU queues before the pipeline directory exists, edit the relevant sample `nextflow.config`. After `CRISPR_Pipeline` exists, edit `CRISPR_Pipeline/nextflow.config` or regenerate the ZIP from the configurator.
